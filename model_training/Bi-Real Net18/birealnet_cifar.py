...
'''
# Bi-Real Net经过修改后的版本，使其能够更好地适配CIFAR数据集
修改：
1. 将网络输入开始部分的卷积核从7*7修改为3*3，并将步长由2更改为1
2. 将网络输入开始部分的池化层去除
3. 将全连接层的输出个数由1000更改为100
'''

@factory
class BiRealNetCifarFactory(ModelFactory):
    ...
    def build(self) -> tf.keras.models.Model:
        ...
        # out = tf.keras.layers.MaxPool2D((3, 3), strides=2, padding="same")(out) # 去除pooling
...
def getBiRealNetCifar():
    model = BiRealNetCifar(
        input_shape=(32,32,3),
        input_tensor=None,
        weights=None,
        include_top=True,
